...
export default function InstructorNutrition() {
  ...
  const [selectedMealPlanCategory, setSelectedMealPlanCategory] =
    useState("All");
  ...
  const [isDeletingRecipeCategory, setIsDeletingRecipeCategory] =
    useState(false);
  const [isDeletingMealPlanCategory, setIsDeletingMealPlanCategory] =
    useState(false);
  ...
  const handleDeleteMealPlanCategory = (category) => {
    ...
    Alert.alert(
      "Delete Meal Plan Category",
      "Are you sure you want to delete this meal plan category?",
      [
        {
          text: "Cancel",
          style: "cancel",
  ...
  const getCategoryRecipes = async (categoryId) => {
    try {
      ...
      const res = await GetApiRequest(`api/recipes/category/${categoryId}`);
  ...
  const createRecipeCategory = async () => {
    try {
      ...
      toast.showToast({
        type: "error",
        message:
          error?.response?.data?.message || "Failed to create recipe category",
        duration: 4000,
      });
    } finally {
  ...
  const updateRecipeCategory = async () => {
    try {
      ...
      toast.showToast({
        type: "error",
        message:
          error?.response?.data?.message || "Failed to update recipe category",
        duration: 4000,
      });
    } finally {
      setCategoryLoading(false);
  ...
  const deleteRecipeCategory = async (categoryId) => {
    try {
      ...
      toast.showToast({
        type: "error",
        message:
          error?.response?.data?.message || "Failed to delete recipe category",
        duration: 4000,
      });
    } finally {
  ...
  const getCategoryMealPlans = async (categoryId) => {
    try {
      ...
      const res = await GetApiRequest(`api/meal-plans/category/${categoryId}`);
    ...
    try {
      ...
      toast.showToast({
        ...
        message:
          ...
          "Failed to create meal plan category",
        duration: 4000,
      });
    } finally {
      setCategoryLoading(false);
  ...
  const updateMealPlanCategory = async () => {
    try {
      setCategoryLoading(true);
      const categoryId =
        editingMealPlanCategory._id || editingMealPlanCategory.id;
      const res = await PutApiRequest(
        `api/meal-plan-categories/${categoryId}`,
        {
          name: categoryName.trim(),
        }
      );
      ...
      toast.showToast({
        ...
        message:
          error?.response?.data?.message ||
          "Failed to update meal plan category",
        duration: 4000,
      });
    } finally {
      setCategoryLoading(false);
  ...
  const deleteMealPlanCategory = async (categoryId) => {
    try {
      ...
      if (res) {
        await getMealPlanCategories();
        if (
          selectedMealPlanCategory ===
          mealPlanCategoriesState.find(
            (cat) => (cat._id || cat.id) === categoryId
          )?.name
        ) {
      ...
      toast.showToast({
        ...
        message:
          error?.response?.data?.message ||
          "Failed to delete meal plan category",
        duration: 4000,
      });
    } finally {
        ...
        <View style={styles.categoryHeader}>
          <View style={styles.categoryActions}>
            <TouchableOpacity
              ...
              onPress={() => {
                const activeCategory = recipeCategoriesState.find(
                  (cat) => cat.active
                );
                if (activeCategory && activeCategory.name !== "All") {
                  ...
                  Alert.alert(
                    "Error",
                    "Please select a recipe category to edit"
                  );
                }
              }}
            >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                ...
                if (activeCategory && activeCategory.name !== "All") {
                  ...
                  Alert.alert(
                    "Error",
                    "Please select a recipe category to delete"
                  );
                }
              }}
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                style={[
                  styles.filterTab,
                  item.active && styles.filterTabActive,
                ]}
      ...
      <View style={styles.categoryHeader}>
        <View style={styles.categoryActions}>
          <TouchableOpacity
            style={[styles.categoryActionButton, styles.editActionButton]}
            onPress={() => {
              const activeCategory = mealPlanCategoriesState.find(
                (cat) => cat.active
              );
              if (activeCategory && activeCategory.name !== "All") {
                ...
                Alert.alert(
                  "Error",
                  "Please select a meal plan category to edit"
                );
              }
            }}
          >
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              ...
              if (activeCategory && activeCategory.name !== "All") {
                ...
                Alert.alert(
                  "Error",
                  "Please select a meal plan category to delete"
                );
              }
            }}
      ...
      >
        ...
        {/* <View style={styles.cardOverlay}>
          ...
          <View style={styles.cardStatsRow}>
            ...
            <Text style={styles.cardStat}>👤 {recipe.servings} serving</Text>
          </View>
        </View> */}
      </TouchableOpacity>
    );
  ...
  const MealPlanCard = ({ mealPlan }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.recipeCard}
        onPress={() => {
          navigation.navigate(RouteName.Client_Meal_Detail, {
            recipe: mealPlan,
          });
        }}
        activeOpacity={0.8}
      >
        ...
        {/* <View style={styles.cardOverlay}>
          ...
          <Text style={styles.cardCategory} numberOfLines={1}>
            {mealPlan.category?.name || "Uncategorized"}
          </Text>

          <Text style={styles.cardStat}>
            📅 {mealPlan.numberOfWeeks} week
            {mealPlan.numberOfWeeks > 1 ? "s" : ""}
          </Text>
        </View> */}
      </TouchableOpacity>
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <TouchableOpacity
        ...
        activeOpacity={0.8}
        onPress={() => navigation.navigate(RouteName.Client_Recipe)}
      >
      ...
      >
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            {t("InstructorNutrition.yourrecipes")}
          </Text>
          <TouchableOpacity
            onPress={() => navigation.navigate(RouteName.Client_Recipe)}
          >
            <Text style={styles.viewAllText}>
              {t("InstructorNutrition.Viewall")}
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.sectionHeader}>
          <Text style={styles.sectionTitle}>
            {t("InstructorNutrition.mealPlan")}
          </Text>
          <TouchableOpacity
            onPress={() => navigation.navigate(RouteName.Client_Meal)}
          >
            <Text style={styles.viewAllText}>
              {t("InstructorNutrition.Viewall")}
            </Text>
